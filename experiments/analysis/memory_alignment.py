#!/usr/bin/env python3
"""Memory Slot Surgery: check alignment between SAE decoder atoms and GDN k*v^T writes.

GDN update: S_t = alpha_t * S_{t-1} + beta_t * k_t * v_t^T
Bilinear SAE decoder: feature i contributes c_i * outer(V_dec[i,0], W_dec[i,0])

Question: do the SAE decoder atoms align with actual k_t * v_t^T writes at
positions where the feature fires?

This script:
  1. Loads Qwen3.5-0.8B and a bilinear SAE checkpoint for L9H4
  2. Runs the model on 100 sequences, hooking the GDN layer to capture k, v, beta, g
  3. Simulates the recurrence to get per-position states
  4. Encodes each position's state through the SAE to get feature activations
  5. For each feature, finds top-50 activating positions, computes cosine alignment
     between V_dec/W_dec and the k_t/v_t at those positions
  6. Reports histograms, top features, and null comparison
"""
...
from sae import BilinearMatrixSAE, build_sae_from_config
...
def report(output: dict, output_dir: str | None = None):
    ...
    print(f"\n{'='*70}")
    print("Memory Slot Surgery: Alignment Report")
    print(f"{'='*70}")
