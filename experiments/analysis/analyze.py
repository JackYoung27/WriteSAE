...
from sae import FlatSAE, MatrixSAE, BilinearMatrixSAE, build_sae_from_config
...
def s0_compare(
    ...
    gained = [{"feature": int(gained_idx[i]), "coefficient": float(trained_acts[gained_idx[i]])}
              for i in gained_order[:20]]

    strengthened_idx = np.where(strengthened_mask)[0]
    str_order = np.argsort(-diff[strengthened_idx])
    strengthened = [{"feature": int(strengthened_idx[i]),
    ...
    suppressed = [{"feature": int(suppressed_idx[i]),
...
def _load_sae(path: str) -> torch.nn.Module:
    ...
    if "model_state_dict" in ckpt:
        ...
        return model.eval()
    raise ValueError(f"Cannot load SAE from {path}: expected model_state_dict key")
