#!/usr/bin/env python3
"""Downstream causal evaluation for Matrix SAEs.

Measures how well SAE-reconstructed GDN recurrent states preserve model behavior.
Split each sequence into a prefix (builds up state) and suffix (measured).
Replace the recurrent state for one layer with the SAE reconstruction, then
compare suffix perplexity against the unmodified baseline.

This isolates the SAE's information loss: if reconstruction is perfect, suffix
perplexity matches baseline exactly.

Architecture note: Qwen3.5's GDN layers only use cached recurrent states when
seq_len == 1 (token-by-token decoding). For multi-token suffix processing, we
patch chunk_gated_delta_rule to inject the initial state directly.
"""
...
from sae import BilinearMatrixSAE, FlatSAE, MatrixSAE, build_sae_from_config

TrainableSAE = FlatSAE | MatrixSAE | BilinearMatrixSAE


# Runtime container holding a loaded SAE plus tag/type/train-mse metadata.
# Distinct from core.types.SAEConfig (training/checkpoint config schema).
class LoadedSAEEntry(TypedDict):
    tag: str
    sae: TrainableSAE
    sae_type: str
...
def load_sae_from_checkpoint(
    ...
    train_mse = ckpt.get("val_mse")
    return sae, cfg, train_mse
...
def evaluate_downstream(
    ...
    layer_idx: int,
    sae_configs: list[LoadedSAEEntry],
    head_idx: int = 0,
