"""PCA and NMF baselines for GDN state reconstruction.

Fits sklearn PCA/NMF on flattened (N, d_k*d_v) states and computes
reconstruction MSE at various component counts. Provides a Pareto
curve of reconstruction quality vs number of components for comparison
against learned SAE representations.

Usage:
    # from Modal:
    modal run run_modal.py --stage baselines --layers 9

    # standalone:
    python baselines.py --data-dir /path/to/states --layer 9
"""
...
from split_utils import make_train_val_indices, select_sample_indices
...
def run_baselines(
    ...
    # -- NMF baseline (only if data is mostly non-negative) --
