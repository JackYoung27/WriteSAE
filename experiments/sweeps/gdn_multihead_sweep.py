#!/usr/bin/env python3
"""GDN multi-layer head sweep: train flat + rank1 SAEs on (layer, head) pairs.

Matches the Mamba-2 multi-head sweep protocol (mamba2_sae_experiment.py) for
cross-architecture comparison. Unit of analysis: (layer, head) pair.

Stages:
1. scan: extract write vectors for ALL heads on 7 GDN layers, compute
   write_vec_top1_energy_fraction, select 16-20 pairs spanning the range
2. extract: save recurrent states for selected pairs (5000 seqs, 1024 seq_len)
3. train: flat + rank1 SAEs on each pair (nf=2048, k=32, 3 seeds, 20 epochs)
4. analyze: correlate write_vec_top1 with rank1_advantage across all pairs

Usage:
    modal run gdn_multihead_sweep.py --stage scan
    modal run gdn_multihead_sweep.py --stage extract
    modal run gdn_multihead_sweep.py --stage train
    modal run gdn_multihead_sweep.py --stage analyze
    modal run gdn_multihead_sweep.py --stage all
"""
...
image = (
    ...
    .add_local_file(
        "results/data/spectral_18layer_correlation.json",
        "/root/spectral_18layer_correlation.json",
        copy=True,
    )
    .env({"MATRIX_SAE_CODE_SHA": CODE_SHA})
)

app = modal.App("gdn-multihead-sweep")
...
HEAD_SCAN_PATH = Path(f"{DATA}/gdn_head_scan_metrics.json")
SELECTED_PAIRS_PATH = Path(f"{DATA}/gdn_head_sweep_pairs.json")
STATE_DIR = Path(f"{DATA}/gdn_states_multihead")
WRITE_SAMPLE_DIR = Path(f"{DATA}/gdn_write_samples_multihead")
CHECKPOINT_DIR = Path(f"{DATA}/gdn_checkpoints_multihead")
RESULTS_PATH = Path(f"{DATA}/gdn_sae_results_multihead.json")


# Utility functions (shared with gdn_head_level_experiment.py)
...
def _load_selected_pairs_payload() -> dict[str, Any]:
    if not SELECTED_PAIRS_PATH.exists():
        raise FileNotFoundError(
            f"Missing selected pairs file: {SELECTED_PAIRS_PATH}. Run stage=scan first."
        )
...
# GDN multi-head write capture (from gdn_head_level_experiment.py)
...
# Pair selection (same algorithm as mamba2_sae_experiment.py)
...
@app.function(
    volumes={DATA: gdn_vol, MODELS: model_vol},
    image=image,
    gpu="A100",
    timeout=14400,
    memory=32768,
...
def scan_heads(
    ...
    tokenizer = AutoTokenizer.from_pretrained(GDN_MODEL_NAME)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    model = AutoModelForCausalLM.from_pretrained(
        GDN_MODEL_NAME, torch_dtype=torch.bfloat16,
    ).to(device)
    model.eval()
    model_vol.commit()
    ...
    # Pre-allocate per-(layer, head) write sample buffers.
    ...
    # Install one capture hook per layer. Each captures all heads.
    captures = {
    ...
    # Load spectral reference for per-layer sv1/sv2 annotation.
    spectral_ref: dict[str, Any] = {}
    spectral_path = Path("/root/spectral_18layer_correlation.json")
    if spectral_path.exists():
        with open(spectral_path) as f:
            spectral_ref = json.load(f)
    spectral_layers = spectral_ref.get("layers", {})

    # Compute write metrics for every (layer, head).
    metrics_by_pair: dict[str, dict[str, float]] = {}
    for layer in layers:
        for head in all_heads:
            pair = (layer, head)
            ...
            # Attach spectral predictor from the 18-layer study if available.
    ...
    # Select pairs spanning the write_vec_top1 range.
    ...
    HEAD_SCAN_PATH.parent.mkdir(parents=True, exist_ok=True)
    with open(HEAD_SCAN_PATH, "w") as f:
        json.dump(scan_payload, f, indent=2)

    # Save selected pairs separately (consumed by later stages).
    selected_payload = {
        "model": GDN_MODEL_NAME,
        "architecture": "gdn",
        "predictor": "write_vec_top1_energy_fraction",
...
def extract_states(
    ...
    # Create memmaps for recurrent states: (n_samples, d_k, d_v) per pair.
    ...
    # Pre-allocate write sample buffers.
    sample_store: dict[tuple[int, int], dict[str, np.ndarray]] = {}
    counts: dict[tuple[int, int], int] = {}
    for layer, head in pair_list:
        pair = (layer, head)
    ...
    # Install write capture hooks on all layers that have selected heads.
    captures = {
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(batches, desc="Extract states")):
            ...
            # use_cache=True to get recurrent states from the cache.
            outputs = model(input_ids=input_ids, use_cache=True)
            cache = outputs.past_key_values

            # Extract end-of-sequence recurrent states from cache.
            for layer, heads in heads_by_layer.items():
                # cache.layers[layer].recurrent_states: (batch, n_heads, d_k, d_v)
                state = cache.layers[layer].recurrent_states.float().cpu().numpy()
                for head in heads:
                    memmaps[(layer, head)][sample_offset : sample_offset + bs] = state[:, head]

            # Extract write factors from the capture hooks.
            for layer, heads in heads_by_layer.items():
                head_results = captures[layer].pop_write_factors()
                _append_head_samples(
                    sample_store,
    ...
    # Finalize: convert memmaps to .npy, clean up tmp files.
    ...
    # Save write samples and compute write metrics.
...
def analyze_results():
    ...
    for pair in selected_pairs:
        ...
        # Load write metrics (recomputed from full extraction).
        ...
        # Load SAE checkpoints for flat + rank1.
...
# Helper: retrieve selected pairs
...
def main(
    stage: str = "all",
    n_pairs: int = N_SWEEP_PAIRS,
    scan_n_sequences: int = SCAN_N_SEQUENCES,
    extract_n_sequences: int = EXTRACT_N_SEQUENCES,
):
    if stage in ("scan", "all"):
        print("=== Stage 1: Scan all heads across 7 GDN layers ===")
        payload = scan_heads.remote(
            n_sequences=scan_n_sequences, n_pairs=n_pairs,
        )
    ...
    if stage in ("extract", "all"):
        ...
        meta = extract_states.remote(n_sequences=extract_n_sequences)
    ...
    if stage in ("train", "all"):
        ...
        n_jobs = len(jobs)
        print(f"Spawned {n_jobs} training jobs ({len(selected_pairs)} pairs x {len(SAE_TYPES)} types x {len(SEEDS)} seeds)")
        results = []
        failures = []
        for idx, job in enumerate(jobs, start=1):
            try:
                results.append(job["call"].get())
        ...
        if failures:
            failure_path = CHECKPOINT_DIR / "training_failures.json"
