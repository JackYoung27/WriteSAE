#!/usr/bin/env python3
"""Steering suppress experiment with 40 prompts (power replication).

Replicates the 20-prompt top3-positive suppress experiment
(memory_slot_suppress_4B_L9_top3_pos_mean_nonzero_clip0_1x_2x_5x_top3_launch.json)
with double the sample size. The original 20 prompts are preserved; 20 new
prompts are appended.

Protocol (identical to original):
  - Model: Qwen/Qwen3.5-4B-Base, layer 9, temperature 0.7, 400 tokens
  - Per-head boundary-differential feature selection (top 3 positive per head)
  - Calibration: mean_nonzero
  - Additive negative push, clamp at zero (clip_min_zero=True)
  - Conditions: baseline, suppress_{1x,2x,5x}, random_suppress_5x
...
Usage:
    modal run --detach steering_suppress_40.py
"""
...
app = modal.App("matrix-sae-suppress-40")
...
def run_suppress_experiment(
    ...
    """Run the memory slot suppression experiment with 40 prompts.

    For each prompt, generate 400 tokens under baseline, one suppress condition
    per requested dose, and one random matched-dose control.

    Feature selection: per-head boundary-differential analysis, keeping only
    positive-differential features (features that activate more at boundaries).
    Top n_boundary_features per head. Push calibrated from mean_nonzero
    boundary activation. Suppression = negate the push, then clamp coefficients
    at zero.
    """
    import sys
    import numpy as np
    ...
    for h in range(n_heads):
        ...
        # Boundary-differential feature selection, then keep requested sign family
    ...
    for strength in suppress_strengths:
        ...
        for h, push_dict in boundary_push_per_head.items():
            # Negate: suppress subtracts the boundary push
    ...
    # Strongest suppress vs matched random
    strongest_strength = max(suppress_strengths) if suppress_strengths else 0.0
    strongest_label = (
        f"suppress_{strongest_strength:.0f}x"
        if float(strongest_strength).is_integer()
    ...
    for subset_name, idx_range in [("original_20", range(0, 20)), ("new_20", range(20, 40))]:
        subset_comp = {}
        for stat in stat_names:
            vals_a = [all_results[i][strongest_label]["stats"][stat] for i in idx_range]
    ...
    result = {
        "experiment": "steering_suppress_40",
        "layer": layer,
        "n_heads": n_loaded,
    ...
    filename = f"steering_suppress_40_4B_L{layer}.json"
    ...
    # ---------------------------------------------------------------
    print(f"\n{'='*80}")
    print(f"STEERING SUPPRESS (40 prompts): L{layer}, {n_loaded} heads, {len(formatted_prompts)} prompts")
    print(f"{'='*80}")
...
def main():
    ...
    local_name = os.path.basename(result.get("output_path", "steering_suppress_40_4B_L9.json"))
    ...
    for comp_name in ["suppress_5x_vs_baseline", "suppress_5x_vs_random_suppress_5x"]:
