#!/usr/bin/env python3
"""Write-reuse metric: predict rank-1 SAE advantage from exact write geometry.

Both GDN and Mamba-2 update matrix-valued recurrent states using rank-1 writes.
This script extracts the true write factors from model internals, computes

    write_reuse_score = 1 / sqrt(erank(C_k) * erank(C_v))

and correlates that score with rank-1 SAE advantage.

The extraction is exact in both cases:
- Mamba-2: capture the true `dBx = v_t k_t^T` factors from the mixer path.
- GDN: capture the exact recurrent write `k_t outer (beta_t * (v_t - S_{t-1} @ k_t))`.

Usage:
    modal run mamba2_write_geometry.py
    modal run mamba2_write_geometry.py --stage extract
    modal run mamba2_write_geometry.py --stage extract-gdn
    modal run mamba2_write_geometry.py --stage analyze
"""
...
image = (
    ...
    .add_local_file(
        "results/data/spectral_audit_mamba2.json",
        "/root/spectral_audit_mamba2.json",
        copy=True,
    )
    .add_local_file(
...
app = modal.App("mamba2-write-geometry")
mamba2_vol = modal.Volume.from_name("mamba2-sae-data-v4", create_if_missing=True)
...
def _correlation_stat(xs: list[float | None], ys: list[float | None]) -> dict[str, Any]:
    ...
    return {"spearman_rho": float(rho), "p_value": float(p), "n": len(valid)}
...
@app.function(
    volumes={MAMBA2_DATA: mamba2_vol, MODELS: model_vol},
    image=image,
    gpu="A100",
    timeout=14400,
    memory=32768,
...
def extract_mamba2_writes(
    layers: list[int] = MAMBA2_LAYERS,
    n_sequences: int = MAMBA2_DEFAULT_N_SEQS,
    seq_len: int = DEFAULT_SEQ_LEN,
    batch_size: int = MAMBA2_BATCH_SIZE,
):
    ...
    write_dir = Path(f"{MAMBA2_DATA}/write_vectors")
...
def _load_mamba2_sae_from_checkpoints() -> dict[str, Any]:
    per_layer: dict[str, Any] = {}
    for layer_idx in MAMBA2_LAYERS:
        ckpt_base = Path(f"{MAMBA2_DATA}/mamba2_checkpoints/layer_{layer_idx}")
        mse_by_type = {}
...
@app.function(
    volumes={MAMBA2_DATA: mamba2_vol},
    image=image,
    timeout=1200,
    memory=16384,
)
def analyze_mamba2():
    ...
    sae_results_path = Path(f"{MAMBA2_DATA}/mamba2_sae_results.json")
    if sae_results_path.exists():
        ...
        per_layer_sae = _load_mamba2_sae_from_checkpoints()

    spectral_ref = {}
    for path in [
        Path(f"{MAMBA2_DATA}/mamba2_states/spectral_audit_mamba2.json"),
        Path("/root/spectral_audit_mamba2.json"),
    ]:
        if path.exists():
            with open(path) as f:
                payload = json.load(f)
    ...
    out_path = Path(f"{MAMBA2_DATA}/write_geometry_mamba2.json")
...
def extract_gdn_writes(
    layers: list[int] = GDN_LAYERS,
    n_sequences: int = GDN_DEFAULT_N_SEQS,
    seq_len: int = DEFAULT_SEQ_LEN,
    batch_size: int = GDN_BATCH_SIZE,
):
    from transformers import AutoModelForCausalLM, AutoTokenizer
    from tqdm import tqdm
    ...
    write_dir = Path(f"{GDN_DATA}/write_vectors")
    write_dir.mkdir(parents=True, exist_ok=True)
    total_tokens = actual_sequences * seq_len
...
def analyze_gdn():
    ...
    gdn_ref_path = Path("/root/spectral_18layer_correlation.json")
    if not gdn_ref_path.exists():
        raise FileNotFoundError("Missing /root/spectral_18layer_correlation.json in image")
    with open(gdn_ref_path) as f:
        gdn_ref = json.load(f)
    ...
    out_path = Path(f"{GDN_DATA}/write_geometry_gdn.json")
    with open(out_path, "w") as f:
        json.dump(output, f, indent=2, default=str)
    gdn_vol.commit()
...
@app.function(
    volumes={MAMBA2_DATA: mamba2_vol, GDN_DATA: gdn_vol},
    image=image,
    timeout=600,
    memory=8192,
)
def combined_summary():
    ...
    results = {}
    m2_path = Path(f"{MAMBA2_DATA}/write_geometry_mamba2.json")
    if m2_path.exists():
        with open(m2_path) as f:
            results["mamba2"] = json.load(f)

    gdn_path = Path(f"{GDN_DATA}/write_geometry_gdn.json")
    if gdn_path.exists():
    ...
    pooled_p = None
    if len(all_ws) >= 3:
        pooled_rho, pooled_p = spearmanr(all_ws, all_adv)
    ...
    for vol_path, vol_ref in [(MAMBA2_DATA, mamba2_vol), (GDN_DATA, gdn_vol)]:
        out_path = Path(f"{vol_path}/write_geometry_combined.json")
        with open(out_path, "w") as f:
...
def main(
    stage: str = "all",
    mamba2_n_seqs: int = MAMBA2_DEFAULT_N_SEQS,
    gdn_n_seqs: int = GDN_DEFAULT_N_SEQS,
    seq_len: int = DEFAULT_SEQ_LEN,
    mamba2_batch_size: int = MAMBA2_BATCH_SIZE,
    gdn_batch_size: int = GDN_BATCH_SIZE,
):
    if stage in ("extract", "extract-mamba2", "all"):
        print("=== Stage 1: Extract Mamba-2 write vectors ===")
        meta = extract_mamba2_writes.remote(
            n_sequences=mamba2_n_seqs,
            seq_len=seq_len,
            batch_size=mamba2_batch_size,
        )
        print(
    ...
    if stage in ("extract-gdn", "all"):
        ...
        meta = extract_gdn_writes.remote(
            n_sequences=gdn_n_seqs,
            seq_len=seq_len,
            batch_size=gdn_batch_size,
        )
        print(
